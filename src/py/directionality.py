...
def hist(data, avg, name, length):
    sns.set_style("whitegrid")
    fig, ax = plt.subplots(figsize=(10, 8))
    plt.hist(data, color="grey", edgecolor="black")
...
def main():
    ...
    for length in range(min(cwalk_length), max(cwalk_length) + 1):
        ...
        if avg_signs:
            ...
            hist(avg_signs_shuffle, round(statistics.mean(avg_signs_shuffle), 2), "shuffle", length)
            hist(avg_signs, round(statistics.mean(avg_signs), 2), "data", length)
